...
import { getViewerMatches } from "@/lib/matches.functions";
import { Fingerprint, type CircleSummary } from "@/components/Fingerprint";
import { DOMAINS } from "@/domain/categories";
...
const matchesQO = (w: string) =>
  queryOptions({
    queryKey: ["viewer-matches", w],
    queryFn: () => getViewerMatches({ data: { wallet: w } }),
  });
...
function WalletPage() {
  ...
  // Build the fixed-order Circle list from the bounded viewer cache: a domain
  // with cached allies is active; every other spoke renders as an honest stub.
  const circles: CircleSummary[] = DOMAINS.map((dom) => {
    const allies = md.domains[dom] ?? [];
    const top = allies[0];
    return top
      ? {
          domain: dom,
          insufficient: false,
          shared_markets: top.sharedMarkets,
          top_ally: { wallet: top.wallet, match_score: top.score },
          top_rival: null,
        }
      : { domain: dom, insufficient: true, shared_markets: 0, top_ally: null, top_rival: null };
  });
  const overallInsufficient = md.tribe.length === 0 && !md.closestMatch;

  const activeCircles = circles.filter((c) => !c.insufficient);
  const blurryCircles = circles.filter((c) => c.insufficient);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-4xl px-6 py-8">
          ...
          <div>
            ...
            <div className="mt-4 space-y-1 text-sm">
              <div>
                ...
                {activeCircles.length} / {circles.length}
              </div>
        ...
        <section className="mt-10">
          <h2 className="mb-3 text-sm font-medium uppercase tracking-wide text-muted-foreground">
            Tribe {overallInsufficient ? "" : `— overall match`}
          </h2>
          {overallInsufficient ? (
            ...
            <div className="overflow-hidden rounded-lg border border-border">
              <table className="w-full text-sm">
                <tbody>
                  {md.tribe.map((m) => (
                    <tr key={m.wallet} className="border-t border-border">
                      <td className="p-3 font-mono text-xs">
                        <a href={`/wallet/${m.wallet}`} className="hover:underline">
                          {short(m.wallet)}
                        </a>
                      </td>
                      <td className="p-3 tabular-nums">{Number(m.score).toFixed(0)}</td>
                      <td className="p-3 text-xs text-muted-foreground">
                        {m.sharedMarkets} shared · {m.agreements} agree · {m.disagreements} disagree
                      </td>
                    </tr>
                  ))}
                </tbody>
