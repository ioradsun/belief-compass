/**
 * The plain-English terms, shared by the /terms route and the in-app center
 * panel so there is exactly one copy of the wording.
 */
      ...
      <div className="mt-8 space-y-9">
        {TERMS_SECTIONS.map((s) => (
          <section key={s.heading}>
            <h2 className="text-[15px] font-semibold text-[var(--text)]">{s.heading}</h2>
            <ul className="mt-3 space-y-2">
              {s.points.map((p) => (
                ...
                >
                  <span
                    aria-hidden
                    className="mt-[9px] h-[3px] w-[3px] shrink-0 rounded-full bg-[var(--text-muted)]"
                  />
                  <span>{p}</span>
                </li>
              ))}
