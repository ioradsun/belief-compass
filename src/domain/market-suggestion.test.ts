...
describe("eligibility", () => {
  it("requires participation across categories", () => {
    expect(
      isParticipationEligible({ meaningfulActions: 5, yesNoDecisions: 3, categoriesTouched: 2 }),
    ).toBe(true);
    expect(
      isParticipationEligible({ meaningfulActions: 5, yesNoDecisions: 3, categoriesTouched: 1 }),
    ).toBe(false);
    expect(
      isParticipationEligible({ meaningfulActions: 4, yesNoDecisions: 3, categoriesTouched: 2 }),
    ).toBe(false);
    expect(
      isParticipationEligible({ meaningfulActions: 9, yesNoDecisions: 2, categoriesTouched: 3 }),
    ).toBe(false);
  });
...
describe("duplicate detection", () => {
  ...
  it("rejects a candidate that duplicates an existing market", () => {
    const v = validateCandidate(
      idea({ question: "Should schools teach children to challenge AI answers?" }),
      CATS,
  ...
  it("reports the nearest existing market", () => {
    expect(
      nearestExisting("Will AI tutors become normal?", ["Should schools allow ChatGPT?"]),
    ).toBeLessThan(0.4);
  });
});
...
describe("validation", () => {
  ...
  it("rejects malformed, compound, promotional and miscategorised questions", () => {
    ...
    expect(
      validateCandidate(idea({ question: `Should schools ${"teach ".repeat(30)}AI?` }), CATS, [])
        .reason,
    ).toBe("too-long");
    expect(
      validateCandidate(idea({ question: "Schools should teach AI literacy." }), CATS, []).reason,
    ).toBe("not-a-question");
    expect(
      validateCandidate(
        idea({ question: "Should schools teach AI and will teachers accept it?" }),
        CATS,
        [],
      ).reason,
    ).toBe("compound");
    expect(
      validateCandidate(
        idea({ question: "Will this market bring guaranteed returns to backers?" }),
        CATS,
        [],
      ).reason,
    ).toBe("banned-phrase");
...
describe("ranking", () => {
  ...
  it("penalises questions close to an existing market", () => {
    ...
    const near = candidateScore(idea(), opp, [
      "Should schools teach kids to challenge AI answers today?",
    ]);
...
describe("copy", () => {
  ...
  it("states the fee entitlement exactly", () => {
    expect(rewardLine(450)).toBe(
      "Create it and earn 4.5% of eligible trading fees generated by the market.",
    );
