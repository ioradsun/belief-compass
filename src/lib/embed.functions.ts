 ...
 *      uses (never trust the client's parse), expand the one short-link form
 *      we support (vm.tiktok.com), and fetch the platform's public oEmbed for
...
/** Public oEmbed endpoints. Instagram's requires an app token — we skip it. */
...
/** Expand vm.tiktok.com / vt.tiktok.com share links to the canonical video URL. */
async function expandShortLink(raw: string): Promise<string | null> {
  let u: URL;
  try {
  ...
  if (h !== "vm.tiktok.com" && h !== "vt.tiktok.com") return null;
  const res = await withTimeout(`https://${h}${u.pathname}`);
  return res?.url ?? null;
}
...
export const resolveEmbed = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ media: EmbedMedia | null; error: string | null }> => {
    let parsed = parseEmbed(data.url);
    if (!parsed) {
      const expanded = await expandShortLink(data.url);
      parsed = expanded ? parseEmbed(expanded) : null;
    }
    if (!parsed) {
      return {
        media: null,
        error: "Only YouTube, Instagram, TikTok, X and Spotify links are supported.",
      };
    }
