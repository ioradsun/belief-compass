...
async function loadFactors(sb: Sb, wallet: string): Promise<DnaFactor[]> {
  ...
  /**
   * A FAILED READ IS NOT A PERSON WITH NO BELIEFS — and this is the exact spot
   * where that distinction was being lost, with the worst possible consequence.
   *
   * The length of this array is `expressedBeliefs`, which `challengeLock` reads
   * to decide whether the reader has taken five sides. `?? []` on a blocked read
   * therefore produced ZERO, and zero produced the locked panel: "Take 5 sides
   * and we'll work out who your people are." Shown to somebody with forty. A
   * completely believable sentence, generated by a request that never answered,
   * telling an established user they had not started.
   *
   * So it throws. The rail already knows how to say that the read failed; what it
   * could not do was tell the difference.
   */
  if (traded.error || expressed.error) {
    throw new Error(
      `dna: belief read failed for ${w} — ${traded.error?.message ?? expressed.error?.message}`,
    );
  }
