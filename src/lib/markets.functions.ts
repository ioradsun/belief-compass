...
/**
 * A wallet's overall Tribe — read straight from the batch matcher's cache
 * (wallet_matches, domain = NULL for overall). No computation here: the
 * dna-matcher job precomputes every wallet, so this is a plain read.
 */
export const getWalletTribe = createServerFn({ method: "GET" })
  .inputValidator((d: { wallet: string }) => z.object({ wallet: z.string().min(3) }).parse(d))
  .handler(async ({ data }) => {
    const wallet = data.wallet.toLowerCase();
    const sb = publicClient();
    const { data: rows } = await sb
      .from("wallet_matches")
      .select("matched_wallet, match_score, shared_markets, agreements, disagreements")
      .eq("wallet", wallet)
      .is("domain", null)
      .order("match_score", { ascending: false })
      .limit(50);
    const matches = rows ?? [];
    return { insufficient: matches.length === 0, wallet, matches };
  });
