...
--
-- market_state is the SINGLE canonical global read model for current market
-- facts (activity, participation, movement, live summary). markets stays the
-- stable metadata + authoritative POV display source. No second market_cards
-- table. Metrics have ONE definition each (see docs/market-state-metrics.md):
--   activity      → canonical events
--   participation → canonical wallet_beliefs
--   display/price → POV (authoritative)
-- Recency uses occurred_at, never ingestion time.

-- ── Normalized read-model fields ─────────────────────────────────────────────
ALTER TABLE public.market_state
  ...
  -- participation (positions) — capital_held_* is COST-BASIS held capital,
  -- distinct from yes_capital_usd/no_capital_usd (POV-reported market cap).
  ...
  -- activity windows (events, by occurred_at)
  ...
  -- tenure / conviction EVIDENCE (factual aggregates only — no classification)
  ...
  -- factual live line (never viewer-specific / opportunity / relevance)
...
-- Product-read + refresh indexes (bounded, non-speculative).
...
-- Aggregation-supporting indexes on the canonical sources.
...
-- ── market_refresh_queue — coalesced dirty markers ───────────────────────────
...
-- Coalesce: one row per market; repeated changes OR the dirty flags together and
-- keep the EARLIEST requested_at so oldest-dirty-age stays meaningful.
...
BEGIN
  ...
  RETURN v_n;
END; $$;
REVOKE ALL ON FUNCTION public.enqueue_market_refresh(bigint[], text) FROM public;
GRANT EXECUTE ON FUNCTION public.enqueue_market_refresh(bigint[], text) TO service_role;

-- ── Aggregation functions (aggregation only — no reducer/conviction math) ─────
-- Activity windows from canonical trade events, by occurred_at. unique wallets
-- are EXACT distinct counts (correct at current scale; not summed hourly).
...
-- Participation from canonical positions (denylisted wallets excluded from
-- believer counts, matching the evaluator). Tenure days from directional_since.
...
-- Transition windows from canonical position-transition events (source='system').
...
-- Atomic claim: take up to p_limit oldest dirty markets (SKIP LOCKED so two
-- workers never claim the same market), delete them from the queue, return them.
-- Failed refreshes re-enqueue via enqueue_market_refresh.
